...
//Nonbonded Potential Selection
	if (!strcmp(pname, "EXPLORE")) {
	  if (sscanf(line, "%*50s %d", &EXPLORE) != 1) ErrorMsg(2, pname);
	  if ( (EXPLORE < 0) || (EXPLORE > 1) ) ErrorMsg(3, pname);
	  continue;
	}
...
//Bin Width for SECONDARY Sampling Direction	
	if (!strcmp(pname, "dWLD2")) {
	  if (sscanf(line, "%*50s %lg", &dWLD2) != 1) ErrorMsg(2, pname);
	  if (dWLD2 > 10.0 || (dWLD1 < 0.0001)) ErrorMsg(3, pname);  //Generic Boundary Limits
	  continue;
	}

//Maximum Boundary for SECONDARY Sampling Direction	
	if (!strcmp(pname, "WLD2min")) {
	  if (sscanf(line, "%*50s %lg", &WLD2min) != 1) ErrorMsg(2, pname);
	  if (WLD2min > 20.0) ErrorMsg(3, pname);  
	  continue;
	}
	
//Minimum Boundary for Primary Sampling Direction	
	if (!strcmp(pname, "WLD2max")) {
	  if (sscanf(line, "%*50s %lg", &WLD2max) != 1) ErrorMsg(2, pname);
	  if (WLD2max < -20.0) ErrorMsg(3, pname);  
	  continue;
	}

//Flatness Criteria
	if (!strcmp(pname, "WLBINVISITS")) {
	  if (sscanf(line, "%*50s %d", &WLBINVISITS) != 1) ErrorMsg(2, pname);
	  if ( (WLBINVISITS < 1) || (WLBINVISITS > 1e10) ) ErrorMsg(3, pname);
	  continue;
	}

//Percentage of DOS cut from initial DOS
	if (!strcmp(pname, "PERCENT_DOS_CUT")) {
	  if (sscanf(line, "%*50s %lg", &PERCENT_DOS_CUT) != 1) ErrorMsg(2, pname);
	  if (PERCENT_DOS_CUT > 1.0) ErrorMsg(3, pname);
	  continue;
	}
...
//Number of samples per bin to satisfy the production run 	
